...
lemma add (hf : IsRestricted R f c) (hg : IsRestricted R g c) : IsRestricted R (f + g) c := by
...
lemma neg (hf : IsRestricted R f c) : IsRestricted R (-f) c := by
...
lemma bddabove (hf : IsRestricted R f c) : BddAbove (convergenceSet R c f) := by
...
def bddabove_nneg (hf : IsRestricted R f c) : ∃ A, A > 0 ∧ ∀ i, ‖coeff R i f‖ * c^i ≤ A := by
...
lemma mul (hf : IsRestricted R f c) (hg : IsRestricted R g c) : IsRestricted R (f * g) c := by
...
/-- The restricted power series over a normed ring `R` with the ultrametric property forms a ring.-/
noncomputable
